...
try:
    import requests    
    from config.eco.Eco import Eco
...
def getContent(url):
    ...
    print(url["url"])
    response = requests.get(url["url"], headers=eco.reqHeader)
...
def fileRead():
    ...
    tmpList = []
    result = os.path.isdir(eco.jsonFilePath)
    
    if result:
        for file in os.listdir(eco.jsonFilePath):
            fileAbstractPath = eco.jsonFilePath + f"/{file}"

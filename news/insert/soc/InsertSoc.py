...
print("--------------------------------------")
print(f"* a) root-path-[Soc] : {PROJ_ROOT_PATH}")
print("--------------------------------------")
...
try:
    import requests    
    from config.soc.Soc import Soc 
...
def getContent(url):
    ...
    print(url["url"])
    response = requests.get(url["url"], headers=soc.reqHeader)
...
def fileRead():
    ...
    tmpList = []
    result = os.path.isdir(soc.jsonFilePath)
    
    if result:
        for file in os.listdir(soc.jsonFilePath):
            fileAbstractPath = soc.jsonFilePath + f"/{file}"

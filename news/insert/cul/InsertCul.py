...
def getContent(url):
    ...
    print(url["url"])
    response = requests.get(url["url"], headers=cul.reqHeader)
...
def fileRead():
    ...
    tmpList = []
    result = os.path.isdir(cul.jsonFilePath)
    
    if result:
        for file in os.listdir(cul.jsonFilePath):
            fileAbstractPath = cul.jsonFilePath + f"/{file}"

...
try:
    import requests    
    from config.world.World import World
...
def getContent(url):
    ...
    print(url["url"])
    response = requests.get(url["url"], headers=world.reqHeader)
...
def fileRead():
    '''
    '''
    global urlList, world
    
    tmpList = []
    result = os.path.isdir(world.jsonFilePath)
    
    if result:
        for file in os.listdir(world.jsonFilePath):
            fileAbstractPath = world.jsonFilePath + f"/{file}"

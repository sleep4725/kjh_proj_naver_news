...
print("--------------------------------------")
print(f"* a) root-path-[It] : {PROJ_ROOT_PATH}")
print("--------------------------------------")
...
def getContent(url):
    ...
    print(url["url"])
    response = requests.get(url["url"], headers=it.reqHeader)
...
def fileRead():
    ...
    tmpList = []
    result = os.path.isdir(it.jsonFilePath)
    
    if result:
        for file in os.listdir(it.jsonFilePath):
            fileAbstractPath = it.jsonFilePath + f"/{file}"
